...
can at least add a small positive vector that direction. I’ll present a
few sample problems here, and let’s be clear from the outset that these
...
interested in hardware design. Aren’t computers getting super, super
...
don’t necessarily make software like computers the absolute fastest for
raw computation. If raw computation is all you care about, then you
don’t need a screen, speakers, and so forth. You also don’t need a
...
| `in_stream`     | Input     | 8            | ASCII character (only either ‘A’ or ‘B’)      |
...
understand bits (1’s and 0’s), so when I type on a keyboard, a keystroke
like the letter ‘A’ needs to be encoded into a bit pattern, transmitted
...
like emojis (👏)</Highlight>. As an example, ‘A’ is encoded into hexadecimal as
`0x41` which in binary is `01000001`. This will be important in a
second.

Back to the question at hand. Let’s go through each signal to make sure
we are on the same page here:

* `clk` - **every computing device** has what’s called a Clock which is a
...
simple instructions you’d have to run to render, say, a video, 5 billion
doesn’t quite seem like so much. Anyway, low-level devices like an FPGA
...
Let’s say that the pattern we want to match is ‘ABAAB’. Then below is
...
The actual gate-level implementation of a D flip-flop doesn’t concern us
...
matching pattern or not. If we don’t then keep reading in data. If we
...
8-bit registers storing the encodings of ‘A’ and ‘B’. If we are matching
‘ABAAB’, then what we want is the 8 bits stored in the left-most
flip-flop to match an ‘A’, the 8 bits in the next flip-flop to match a
‘B’ and so on. Think about a logic gate/device you may know of that
...
along with either an ‘A’ or a ‘B’, then AND the result from all of
...
But wait! It turns out that this solution doesn’t satisfy the timing
...
“magic” bit patterns corresponding to any of the characters that could
...
smallest form, on the scale of megabytes (1000’s of kB)^^. How about
...
It’s all well and good to be able to, as we did above, consider a
...
upends the original solution, and 2) this process doesn’t scale well for
...
Humans don’t think very well in terms of 1’s and 0’s, but **we are
pretty good at mapping problems into some sort of procedural
algorithm**. So why don’t we “design” our hardware in software and then
let the computer transpile this into actual hardware. What a great
idea…this is precisely what languages like Verilog and VHDL do. I might
do a Verilog tutorial at a later date, but for now I’ll assume some
...
This algorithm looks innocent enough, but it’s worthwhile pondering what
...
four bits, we need really only figure out what’s going on for one bit,
...
is in fact correct. It’s also worth considering how the synthesis would
