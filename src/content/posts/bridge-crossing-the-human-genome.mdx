...
An NP type problem is one for which **no** such polynomial-time solver exists, but for which there exists a polynomial-time <Highlight>validator' /> for solutions to the problem. That is, I might not be able to **find** a Hamiltonian cycle in a graph $$G$$ efficiently, but if you **give** me an ordering of the vertices, I can <Highlight text='efficiently check whether or not the ordering is a Hamiltonian cycle or not</Highlight>. Last remark here: the "complete" part of NP-complete means that any other NP problem can, in some efficient (read: polynomial time) fashion be translated into this problem of Hamiltonian cycles. This is of interest to computer scientists because there is an open problem $$P=NP$$ which investigates whether in fact the green and red circles above are in fact one and the same. One approach to solving this is to find some NP-complete problem like Hamiltonian cycles, show that it is in fact a P-type problem, and via the NP-completeness of it conclude that all other NP problems are in fact P.
